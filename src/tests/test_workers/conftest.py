...
from src.models.other import Jackpot, JackpotType, GameStatus
...
async def jackpot(
    ...
    await db.execute(
        delete(Jackpot).where(Jackpot.name == "Test Jackpot")
    )
    await db.commit()
    ...
    game = Jackpot(
        name="Test Jackpot",
        currency_id=currency.id,
        _type=JackpotType.GLOBAL,
        percentage=10,
        status=GameStatus.PENDING,
        scheduled_datetime=datetime(2026, 1, 30, 12, 57, 40)
    )
    db.add(game)
    await db.commit()
